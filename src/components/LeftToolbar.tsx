...
interface LeftToolbarProps {
  currentStepType: ProcessStepType | "nail-shape";
  ...
  return (
    <aside className="w-60 bg-[#303030] border-l border-[#4A4A4A] flex flex-col select-none text-xs h-[calc(100vh-44px)]">
      ...
      <div className="flex-1 overflow-y-auto p-2.5 space-y-3.5">
        {/* 0. NAIL SHAPE TOOLS */}
        {currentStepType === "nail-shape" && (
          <div className="space-y-3">
            <div>
              <div className="text-[11px] text-[#A8A8A8] font-medium mb-1.5 flex items-center justify-between">
                <span>目标甲型选择</span>
                <span className="text-[10px] text-[#5E7EB8]">十指联动</span>
              </div>
                ...
                {NAIL_SHAPES.map((shape) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full p-2 rounded text-left border flex items-center space-x-2.5 transition-colors ${
                        isSelected
                    ...
                    >
                      <div
                        className={`w-5 h-8 rounded-xs border flex items-center justify-center p-0.5 shrink-0 ${
                          isSelected
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          ...
                          {isSelected && (
                            <Check className="w-3.5 h-3.5 text-[#5E7EB8] shrink-0" />
                          )}
                        </div>
                        <div className="text-[10px] text-[#888888] font-mono mt-0.5 flex items-center justify-between">
                          <span>{shape.enName}</span>
                          <span className="px-1 rounded bg-[#202020] text-[#A8A8A8] text-[9px]">
                            {shape.tag}
                          </span>
                        </div>
                      </div>
            ...
            {/* Shape Characteristic Details */}
            <div className="p-2.5 bg-[#252525] rounded border border-[#404040] space-y-1.5 text-[11px]">
              <div className="text-[10px] font-bold text-[#E5E5E5] flex items-center space-x-1">
              ...
              <p className="text-[10px] text-[#888888] leading-relaxed">
                {
                  NAIL_SHAPES.find(
                    (s) =>
                      s.id === (targetShape === "squoval" ? "coffin" : targetShape)
                  )?.description
                }
              </p>
            </div>
            ...
            {/* Production Info */}
            <div className="p-2 bg-[#202020] rounded border border-[#3E3E3E] text-[10px] text-[#777777] leading-relaxed">
              💡 提示：甲型定义了 2D 裁切与 3D 曲面基底。切换后，所有装饰图案将自动映射到新甲型轮廓内。
            </div>
          </div>
        )}
